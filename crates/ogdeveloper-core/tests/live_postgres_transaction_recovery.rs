...
async fn live_postgres_schema_queries_can_recover_after_transaction_abort() {
    ...
    let pool = ogdeveloper_core::db::postgres::connect(&url, Duration::from_secs(5)).await.expect("connect");
    let schema = format!("dbx_tx_recovery_{}", uuid::Uuid::new_v4().simple());

    ogdeveloper_core::db::postgres::execute_batch(
        &pool,
        &[
    ...
    ogdeveloper_core::db::postgres::execute_query_with_schema(&pool, &schema, "BEGIN")
        .await
        .expect("begin transaction");

    let failed = ogdeveloper_core::db::postgres::execute_query_with_schema(
        &pool,
        &schema,
        "UPDATE missing_tx_probe SET value = 2",
    )
    .await
    ...
    let aborted =
        ogdeveloper_core::db::postgres::execute_query_with_schema(&pool, &schema, "SELECT value FROM tx_probe")
            .await
            .expect_err("session should remain aborted before rollback");
    assert!(aborted.contains("current transaction is aborted"));

    ogdeveloper_core::db::postgres::execute_query_with_schema(&pool, &schema, "ROLLBACK")
        .await
        .expect("rollback should recover the session");

    let recovered = ogdeveloper_core::db::postgres::execute_query_with_schema(
        &pool,
        &schema,
    ...
    ogdeveloper_core::db::postgres::execute_batch(&pool, &[format!("DROP SCHEMA IF EXISTS \"{schema}\" CASCADE")])
        .await
