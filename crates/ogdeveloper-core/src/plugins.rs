...
impl PluginRuntimeEnv {
    ...
    pub fn apply_to(&self, command: &mut Command) {
        ...
        // Pass both names so cached upstream JDBC launchers honor the new settings.
        for suffix in ["JAVA_BIN", "JAVA_OPTS"] {
            let current = format!("OGDEVELOPER_{suffix}");
            let legacy = format!("DBX_{suffix}");
            let value = self
                .get(&current)
                .map(str::to_owned)
                .or_else(|| std::env::var(&current).ok())
                .filter(|v| !v.is_empty())
                .or_else(|| {
                    self.get(&legacy)
                        .map(str::to_owned)
            ...
            if let Some(value) = value {
                command.env(&current, &value).env(&legacy, value);
            }
        }
    }
}
...
#[cfg(test)]
mod java_env_compat_tests {
    use super::*;

    #[test]
    fn current_java_settings_are_forwarded_to_both_launcher_generations() {
        let env = PluginRuntimeEnv::default()
            .with_var("DBX_JAVA_BIN", "legacy-java")
            .with_var("OGDEVELOPER_JAVA_BIN", "current-java")
            .with_var("DBX_JAVA_OPTS", "-Xmx128m")
            .with_var("OGDEVELOPER_JAVA_OPTS", "-Xmx256m");
        let mut command = Command::new("unused");
        env.apply_to(&mut command);
        let vars: std::collections::HashMap<_, _> = command.as_std().get_envs().collect();
        for key in ["OGDEVELOPER_JAVA_BIN", "DBX_JAVA_BIN"] {
            assert_eq!(vars.get(std::ffi::OsStr::new(key)), Some(&Some(std::ffi::OsStr::new("current-java"))));
        }
        for key in ["OGDEVELOPER_JAVA_OPTS", "DBX_JAVA_OPTS"] {
            assert_eq!(vars.get(std::ffi::OsStr::new(key)), Some(&Some(std::ffi::OsStr::new("-Xmx256m"))));
        }
    }
}
