...
// bundles the official openGauss JDBC driver (org.opengauss.Driver) and
...
fn jdbc_maven_resolver_executable(plugin_dir: &Path) -> PathBuf {
    #[cfg(windows)]
    {
        preferred_maven_launcher(plugin_dir, "ogdeveloper-maven-resolver.bat", "dbx-maven-resolver.bat")
    }
    #[cfg(not(windows))]
    {
        preferred_maven_launcher(plugin_dir, "ogdeveloper-maven-resolver", "dbx-maven-resolver")
    }
}
...
fn install_jdbc_plugin_zip(bytes: &[u8], plugin_dir: &Path) -> Result<(), String> {
    ...
    #[cfg(unix)]
    {
        ...
        for executable in
            ["ogdeveloper-jdbc-plugin", "ogdeveloper-maven-resolver", "dbx-jdbc-plugin", "dbx-maven-resolver"]
        {
