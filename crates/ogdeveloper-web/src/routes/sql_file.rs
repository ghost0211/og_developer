...
use ogdeveloper_core::sql;
use ogdeveloper_core::sql::{SqlFileProgress, SqlFileRequest, SqlFileStatus};
...
pub async fn preview_sql_file(
    ...
    if let Some(field) = multipart.next_field().await.map_err(|e| AppError::from(e.to_string()))? {
        ...
        let bootstrap_analysis = ogdeveloper_core::sql_file_import::mysql_like_sql_file_bootstrap_analysis(&content);
...
pub async fn execute_sql_file(
    ...
    if let Some(name) = ogdeveloper_core::query::connection_readonly_name(&state.app, &req.connection_id).await {
        return Err(AppError::from(format!(
...
mod tests {
    ...
    fn uploaded_sql_paths_are_unique_and_keep_the_extension() {
        let data_dir = std::env::temp_dir().join(format!("ogdeveloper-web-sql-file-test-{}", uuid::Uuid::new_v4()));
    ...
    fn expired_sql_file_uploads_are_removed() {
        let tmp_dir = std::env::temp_dir().join(format!("ogdeveloper-web-sql-file-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp_dir).unwrap();
