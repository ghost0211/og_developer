...
fn reject_web_unsupported_ai_provider(config: &AiConfig) -> Result<(), AppError> {
    if ogdeveloper_core::ai::is_cli_provider(&config.provider) {
...
pub async fn ai_complete(
    ...
    ogdeveloper_core::ai::merge_global_max_retries(
        &mut request.config,
        state.app.storage.load_max_retries().await.unwrap_or(ogdeveloper_core::ai::DEFAULT_MAX_RETRIES),
    );
    let result = ogdeveloper_core::ai::complete(&request).await.map_err(AppError::from)?;
    Ok(Json(result))
}
...
pub async fn ai_test_connection(
    ...
    let result = ogdeveloper_core::ai::test_connection_core(&config).await.map_err(AppError::from)?;
    Ok(Json(result))
}
...
pub async fn ai_list_models(
    ...
    let result = ogdeveloper_core::ai::list_models_core(&config).await.map_err(AppError::from)?;
    Ok(Json(result))
}
...
pub async fn ai_resolve_model_effort(
    ...
    let result =
        ogdeveloper_core::ai::resolve_model_effort_core(&config, &body.model_id).await.map_err(AppError::from)?;
    Ok(Json(result))
}
...
pub async fn ai_cancel_stream(Json(body): Json<AiCancelStreamRequest>) -> Result<Json<bool>, AppError> {
    let result = ogdeveloper_core::ai::cancel_stream(&body.session_id).await;
    Ok(Json(result))
}
...
pub async fn ai_stream(
    ...
    tokio::spawn(async move {
        let result = ogdeveloper_core::ai::stream(&sid, &request, &cancelled, |chunk: AiStreamChunk| {
...
pub async fn ai_agent_stream(
    ...
    let production_database = state
        ...
        .is_some_and(|config| ogdeveloper_core::production_safety::is_production_database(config, &body.database));
    ...
    let max_retries = state.app.storage.load_max_retries().await.unwrap_or_else(|err| {
        ...
        ogdeveloper_core::ai::DEFAULT_MAX_RETRIES
    });
    ...
    let (_allow_write_sql, confirmed_write_sql) = ogdeveloper_core::agent_tools::verify_confirmed_target(
    ...
    ogdeveloper_core::ai::merge_global_max_retries(&mut req_config, max_retries);
...
mod tests {
    ...
    async fn web_test_connection_respects_global_max_retries_zero() {
        ...
        let dir = std::env::temp_dir().join(format!("ogdeveloper-web-intg-max-retries-{}", uuid::Uuid::new_v4()));
