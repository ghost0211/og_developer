...
1. ~~裁剪定型（名字/品牌/入口收敛）~~ ✅ 已完成（og developer / com.ogdeveloper.app / 白名单裁剪）
2. ~~JDBC 内嵌~~ ✅ 已完成（连接模式选择器，默认官方 JDBC 驱动，自动从 Maven 下载 opengauss-jdbc:6.0.0，真机验证通过）
3. 兼容模式感知（A/B/C/PG/M 探测与方言切换）
