...
<Callout type="warn">This exports OG Developer connection configuration, not business database schema or data. Use [Database Export](/en/docs/database-export) for database contents.</Callout>
...
- OG Developer stores ordinary connection fields separately from secrets such as database passwords, SSH passwords, SSH key passphrases, and full connection strings; export brings them together only inside the encrypted export file.
...
| Saved SQL, UI/editor settings, and sidebar layout                      | No (desktop-local settings) |
