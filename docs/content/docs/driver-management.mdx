...
OG Developer uses a hybrid driver architecture: built-in Rust drivers, standalone native Agents, and Java/JDBC Agents.

## Driver Architecture

| Driver Type       | How It Works                         | Best For                                                                                                                                                        |
| ----------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native (built-in) | Rust drivers compiled into OG Developer       | MySQL, PostgreSQL, SQLite, SQL Server, Redis, MongoDB, ClickHouse, and more                                                                                      |
| Native Agent / external driver | Standalone Go/Rust process or sidecar managed by OG Developer | Oracle, KingBase, XuguDB, DuckDB, and connections such as RabbitMQ that require a separate runtime |
| JDBC Agent        | Java subprocess managed by OG Developer       | Databases that require JDBC: GaussDB, openGauss, DM, HighGo, Vastbase, UXDB, Trino, Hive, DB2, Informix, Neo4j, TDengine, YashanDB, GoldenDB, Kylin, SunDB, and more |
...
<Steps>
  ...
  <Step>
    ...
    OG Developer downloads the native Agent executable or JDBC Agent JAR and any required runtime. Progress is shown during download.
  </Step>
  <Step>
    ### Create a Connection
...
JDBC agent drivers require a Java Runtime Environment. OG Developer includes JRE management:

- **System JRE**: OG Developer can use an existing Java installation on your system
- **Managed JRE**: OG Developer can download and manage its own JRE, isolated from your system Java
...
OG Developer checks for driver updates periodically:
...
For environments without internet access, OG Developer supports offline driver installation:

1. Download the OG Developer offline driver ZIP matching the target OS and CPU architecture on another machine
...
The ZIP contains the version metadata and native agent for that platform. OG Developer installs it as `%USERPROFILE%\.dbx\agents\drivers\kingbase\agent.exe`, without requiring users to unblock or manually copy the executable.

## Plugin Updates

For databases supported by the [JDBC Plugin](/en/docs/plugins), OG Developer shows update notices when a new plugin version is available. Plugin updates follow the same install flow as built-in agent drivers.
