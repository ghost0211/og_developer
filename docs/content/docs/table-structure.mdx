...
The table structure editor keeps single-table metadata and a change draft in a persistent tab. OG Developer enables only operations that the active database capability can generate reliably and shows the final DDL before save.
...
- Some databases save only the local display order in OG Developer without changing physical structure
...
Edits do not immediately change the database. Before save, OG Developer generates highlighted dialect-specific SQL such as:
...
<Steps>
  ...
  <Step>
    ...
    Execute after review. Production scope enters production-write confirmation again, and OG Developer reloads structure after success.
...
Do not assume that every DDL feature is editable because a database can connect. Use the [openGauss connection guide](/en/docs/opengauss) and the actual editor controls as the current source of truth.
...
- Use [Database Export](/en/docs/database-export) before risky changes
