...
description: Organize connections, search database objects, open data and structure, and run database-specific actions from the OG Developer sidebar.
---

The object tree is the main navigation surface in OG Developer. It displays more than databases, schemas, tables, and columns: it also manages connection groups, visible scope, object search, pins, copy/paste, schema actions, specialized workspaces, and administration views.
...
| Type | Example tree |
| --- | --- |
| MySQL-compatible (B mode) | Connection → database → table/view → column/index/foreign key/trigger |
| PostgreSQL, openGauss, Oracle, SQL Server, and others | Connection → database or service → schema → object |
...
Supported connection imports can retain group information. When sidebar layouts move between devices, OG Developer remaps connection IDs.
...
OG Developer does not force every system into a relational tree. For example:
