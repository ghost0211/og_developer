---
title: JDBC Plugin
description: Install the OG Developer JDBC plugin, import vendor JDBC driver JARs, and create JDBC connections.
---

OG Developer uses native Rust database drivers by default. JDBC is optional: install the JDBC plugin when you need to connect to a database that OG Developer does not support natively, or when you specifically need a vendor JDBC driver.

<Callout type="info">The JDBC plugin is not bundled with the main OG Developer app, and it does not include vendor JDBC drivers. Install the OG Developer JDBC plugin first, then import the database vendor's JDBC driver JAR.</Callout>

<Callout type="warn">A successful JDBC connection does not imply every capability of a native OG Developer profile. Object trees, source definitions, structure editing, import, transfer, and admin views depend on JDBC metadata, the detected effective database type, and the driver capability manifest. Unknown engines run with generic JDBC capabilities.</Callout>
...
Prefer built-in OG Developer database types first, such as MySQL, PostgreSQL, SQLite, Redis, MongoDB, DuckDB, ClickHouse, SQL Server, Oracle, DM, and GaussDB.

Use JDBC when:

- The target database is not built into OG Developer
...
| Item                   |         Required | Notes                                                                                  |
| ---------------------- | ---------------: | -------------------------------------------------------------------------------------- |
| OG Developer JDBC plugin        |              Yes | Installed from OG Developer settings; lets OG Developer call JDBC                                        |
...
If the command is missing, install Java first. OG Developer checks `JAVA_HOME` first, then common system locations and `java` from `PATH`.
...
<Tabs groupId="java-install" items={['macOS', 'Windows', 'Linux']}>
  ...
  <Tab value="Windows">
    Install Temurin, Oracle JDK, or Microsoft Build of OpenJDK. Restart OG Developer after installation and verify:
...
## Step 2: Install the OG Developer JDBC Plugin

In OG Developer Desktop:

1. Open **Settings**
2. Go to **JDBC Drivers**
3. Click **Install JDBC Plugin**

After installation, OG Developer shows a status like:

```text
Installed: v0.1.0
```

The OG Developer JDBC plugin has its own version. It does not have to match the OG Developer app version. Updating OG Developer does not automatically install the JDBC plugin, and if you uninstall it manually, OG Developer will not reinstall it behind your back.

## Step 3: Download the Vendor JDBC Driver

OG Developer does not download vendor JDBC drivers automatically. Download the `.jar` file from the database vendor website or a Maven repository.
...
You can also paste a local JAR path and import it. OG Developer copies imported JARs into the app data directory, and new JDBC connections can select them from the driver dropdown.
...
OG Developer already has a built-in DM ODBC connection type. If you prefer the official Dameng JDBC driver, use the JDBC plugin.

1. Install the OG Developer JDBC plugin
2. Install Java
...
Check whether your network can access OG Developer GitHub Releases. OG Developer tries proxy URLs and the original GitHub URL, but public proxies can still be unavailable. Try another network if GitHub is blocked.
...
If the terminal works but OG Developer does not, restart OG Developer. On macOS, if Java is only installed under a Homebrew path, configure `JAVA_HOME`.
...
Click **Uninstall** in **Settings → JDBC Drivers**. OG Developer removes the JDBC plugin itself but keeps your imported vendor driver JARs, so you do not have to import them again if you reinstall the plugin later.

## For Developers: Plugin Protocol

OG Developer starts the plugin executable and exchanges JSON Lines requests and responses over stdin/stdout. After a connection is established, OG Developer reuses the same plugin process for queries and metadata requests on that connection.
...
The current OG Developer app supports plugin protocol version `1`. OG Developer rejects a plugin whose manifest declares a different protocol version, and each plugin request has a 30 second timeout so a hung driver process does not block the app indefinitely.
