...
OG Developer provides two comparison workflows:
...
<Steps>
  ...
  <Step>
    ...
    OG Developer reads metadata and DDL, builds an object tree, and marks creations, deletions, modifications, and dependencies. Select the objects that should actually deploy.
...
OG Developer attempts to preserve database-specific semantics rather than normalizing everything to generic DDL, including:
...
A supported connection does not mean every object can compare or deploy. Use the [openGauss connection guide](/en/docs/opengauss) and capability messages in the result.
...
<Steps>
  ...
  <Step>
    ...
    OG Developer reads both sides and marks source-only, target-only, and changed rows. Batch mode shows current table, progress, and per-table failures.
  ...
  <Step>
    ...
    OG Developer generates target-dialect `INSERT`, `UPDATE`, and `DELETE` statements. Review statement count and target before execution.
...
<Cards>
  ...
  <Card title="Export the database" href="/en/docs/database-export">
    Create a reviewable SQL export before risky synchronization.
  </Card>
