...
Field lineage helps answer where a column comes from, where it is used, and what a change might affect. OG Developer combines database metadata with local query history and labels evidence strength instead of treating every matching name as a confirmed dependency.
...
OG Developer uses executed queries as supporting evidence, including:
...
History only represents SQL executed or saved in OG Developer. It does not cover all application code, ETL, BI, or other clients.
...
- Without declared foreign keys, OG Developer can only provide view, history, and same-name clues
...
- Use the [openGauss connection guide](/en/docs/opengauss) for the supported engine surface
