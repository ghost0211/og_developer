...
Development builds can run alongside an installed og developer instance. Both use the same local DBX data, so connections and history are available in each window. Avoid changing the same connection, saved SQL, or global setting in both windows at once.
...
openGauss fixes that stay close to upstream patterns can also be submitted to [t8y2/dbx](https://github.com/t8y2/dbx) as pull requests — see the `opengauss-fixes` branch in this repository for a clean, self-contained fix set ready for that purpose.
