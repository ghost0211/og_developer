...
description: Set up OG Developer from scratch, run the desktop app, make your first change, test it, and open a pull request.
---

This tutorial is for first-time OG Developer contributors. By the end, you will have:

1. Installed Node.js, pnpm, Rust, and native build dependencies
2. Forked, cloned, and run OG Developer Desktop
...
Open [OG Developer Issues](https://github.com/ghost0211/og_developer/issues) and choose an issue that has no assignee and no active contributor in its comments. Good first contributions have one of these properties:
...
OG Developer Desktop uses Tauri, Vue, and Rust. The repository currently requires:
...
See the [official Tauri prerequisites](https://v2.tauri.app/start/prerequisites/) for complete platform details. OG Developer also uses ODBC development libraries.
...
Open [ghost0211/og_developer](https://github.com/ghost0211/og_developer) on GitHub and click **Fork**.

Replace `<your-github-name>` with your account name:

```bash
git clone https://github.com/<your-github-name>/og_developer.git
cd og_developer
git remote add upstream https://github.com/t8y2/dbx.git   # upstream dbx, optional: sync upstream fixes
git remote -v
```
...
## 4. Run OG Developer for the First Time
...
Development builds can run alongside an installed OG Developer instance. Both use the same local OG Developer data, so connections and history are available in each window. Avoid changing the same connection, saved SQL, or global setting in both windows at once.
...
A successful run opens the OG Developer desktop window without a compiler error in the terminal. Complete three quick checks:

1. Open Settings
2. Create or open a local test connection
3. Close and start the app again to confirm the setup is repeatable

If port `1420` is already in use, stop the previous OG Developer/Vite development process and restart.
...
The service ports listen on `127.0.0.1` by default. Set `DB_BIND_ADDRESS=0.0.0.0`, choose a strong `DB_PASSWORD`, and use firewall controls before exposing a host to a shared network. For this fork, openGauss development uses a local Docker instance (`openGauss-lite` 7.0.0-RC3, port 5432); see the `db-*` targets in the [Makefile](https://github.com/ghost0211/og_developer/blob/main/Makefile) for the test database recipes.
...
| `crates/ogdeveloper-core/` | Connections, queries, metadata, and shared Rust database logic |
| `crates/ogdeveloper-web/` | Docker/Web backend |
| `packages/app-tests/` | Tests for shared frontend logic |
| `packages/cli/` | CLI packages (inherited from dbx, not the product focus) |
...
Shared Rust logic is mainly in `crates/ogdeveloper-core/`; desktop commands are in `src-tauri/`. When DuckDB is unrelated, start with the fast checks:
...
Agents are separate processes that communicate with OG Developer through stdin/stdout JSON-RPC. Java/JDBC agents normally use JDK 21; native agents use Go or Rust. Read these first:

- [`agents/README.md`](https://github.com/ghost0211/og_developer/blob/main/agents/README.md)
- [`agents/docs/agent-authoring.md`](https://github.com/ghost0211/og_developer/blob/main/agents/docs/agent-authoring.md)
...
A successful build does not mean the local OG Developer app is using the new code. OG Developer runs `.dbx/agents/drivers/<db_type>/agent.jar` under the user home directory, so back up and replace the runtime JAR:
...
Restart OG Developer or disconnect and reconnect the database so the old agent process exits and the new JAR is loaded. Repeat the issue's reproduction flow against the real database version.
...
Open your fork on GitHub and click **Compare & pull request**. Set the target repository to `ghost0211/og_developer` and the target branch to `main`.
...
- Read the repository [CONTRIBUTING.md](https://github.com/ghost0211/og_developer/blob/main/CONTRIBUTING.md)
