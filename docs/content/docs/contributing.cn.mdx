---
title: 从源码编译与参与贡献
description: 从零搭建 OG Developer 开发环境，运行桌面版，完成第一次修改、测试并提交 Pull Request。
---

这是一份面向第一次参与 OG Developer 开发者的完整教程。跟着本页操作，你将完成：

1. 准备 Node.js、pnpm、Rust 和系统编译依赖
2. Fork、克隆并运行 OG Developer 桌面版
3. 找到要修改的代码并完成一次小改动
4. 运行与改动范围匹配的检查和测试
...
打开 [OG Developer Issues](https://github.com/ghost0211/og_developer/issues)，选择一个尚未分配、评论中也没有人正在处理的 Issue。第一次贡献优先考虑：
...
OG Developer 桌面版基于 Tauri、Vue 和 Rust。仓库当前要求：
...
Tauri 的完整系统要求可以参考 [Tauri 官方前置依赖文档](https://v2.tauri.app/start/prerequisites/)。OG Developer 还需要 ODBC 开发库。
...
先在 GitHub 打开 [ghost0211/og_developer](https://github.com/ghost0211/og_developer)，点击右上角 **Fork** 创建自己的仓库副本。

将下面的 `<你的 GitHub 用户名>` 替换为你的账号：

```bash
git clone https://github.com/<你的 GitHub 用户名>/og_developer.git
cd og_developer
git remote add upstream https://github.com/t8y2/dbx.git   # 上游 dbx，可选：用于同步上游修复
git remote -v
```
...
开发版可以与已安装的 OG Developer 同时运行。两者使用同一份本地 OG Developer 数据，因此连接和历史记录会同时可用；请避免在两个窗口中同时修改同一个连接、收藏 SQL 或全局设置。
...
成功时会出现 OG Developer 桌面窗口，并且终端中没有编译错误。建议完成三个快速检查：

1. 打开设置页面
2. 创建或打开一个本地测试连接
3. 关闭窗口后再次启动，确认开发环境可重复运行

如果端口 `1420` 已被占用，先关闭旧的 OG Developer/Vite 开发进程再重新启动。
...
服务端口默认监听 `127.0.0.1`；在共享网络中暴露主机前，请设置 `DB_BIND_ADDRESS=0.0.0.0`、使用强 `DB_PASSWORD` 并配置防火墙。本 fork 的 openGauss 开发使用本地 Docker 实例（`openGauss-lite` 7.0.0-RC3，端口 5432）；测试库配方见 [Makefile](https://github.com/ghost0211/og_developer/blob/main/Makefile) 中的 `db-*` 目标。
...
| `crates/ogdeveloper-core/` | 数据库连接、查询、元数据和共享 Rust 逻辑 |
| `crates/ogdeveloper-web/` | Docker/Web 后端 |
| `packages/app-tests/` | 前端共享逻辑测试 |
| `packages/cli/` | CLI 包（继承自 dbx，非产品重点） |
...
Rust 共享逻辑主要位于 `crates/ogdeveloper-core/`，桌面命令位于 `src-tauri/`。不涉及 DuckDB 时先运行快速检查：
...
Agent 是独立进程，通过 stdin/stdout JSON-RPC 与 OG Developer 通信。Java/JDBC Agent 通常使用 JDK 21；原生 Agent 使用 Go 或 Rust。开始前先阅读：

- [`agents/README.zh-CN.md`](https://github.com/ghost0211/og_developer/blob/main/agents/README.zh-CN.md)
- [`agents/docs/agent-authoring.zh-CN.md`](https://github.com/ghost0211/og_developer/blob/main/agents/docs/agent-authoring.zh-CN.md)
...
仅仅构建成功还不代表本地 OG Developer 使用了新代码。OG Developer 运行时读取用户目录下 `.dbx/agents/drivers/<db_type>/agent.jar`，因此必须备份并替换运行时 JAR：
...
替换后重启 OG Developer，或者断开并重新连接数据库，确保旧 Agent 进程退出并加载新 JAR。然后使用 Issue 对应的真实数据库版本重新执行复现步骤。
...
| CLI/Node Core | `pnpm test:packages` |
...
在 GitHub 打开你的 Fork，点击 **Compare & pull request**，目标仓库选择 `ghost0211/og_developer`，目标分支选择 `main`。
...
- 查看仓库根目录的 [CONTRIBUTING.zh-CN.md](https://github.com/ghost0211/og_developer/blob/main/CONTRIBUTING.zh-CN.md)
