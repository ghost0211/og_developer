...
开发版可以与已安装的 og developer 同时运行。两者使用同一份本地 DBX 数据，因此连接和历史记录会同时可用；请避免在两个窗口中同时修改同一个连接、收藏 SQL 或全局设置。
...
贴近上游模式的 openGauss 修复也可以作为 PR 提交给 [t8y2/dbx](https://github.com/t8y2/dbx) 回馈上游——本仓库的 `opengauss-fixes` 分支就是为此准备的干净修复集。
