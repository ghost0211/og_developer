---
title: Query Editor
description: Write, understand, parameterize, execute, analyze, and reuse SQL in OG Developer.
---

The query editor is the SQL workspace in OG Developer. It combines dialect-aware highlighting and completion, semantic diagnostics, parameter input, execution progress, multiple results, execution plans, query history, the SQL library, and AI assistance in one tab.
...
When text is selected, OG Developer prioritizes the selection. Otherwise, the current-statement and execution-target settings determine the scope. Shortcuts can be changed under [Keyboard Shortcuts](/en/docs/keyboard-shortcuts).
...
<Callout type="info">Refresh connection metadata after creating or changing objects. OG Developer caches and lazy-loads table structure to reduce repeated requests in large schemas.</Callout>
...
Before execution, OG Developer can recognize common placeholders and open a parameter-input dialog:
...
| --- | --- | --- |
| SQL Library | Saving and searching reusable queries in folders inside OG Developer | Full local SQL Library management is available on Desktop; cloud-sync scope depends on settings |
