#[tauri::command]
pub fn prepare_schema_diff(
    options: ogdeveloper_core::schema_diff::SchemaDiffPreparationOptions,
) -> Result<ogdeveloper_core::schema_diff::SchemaDiffPreparation, String> {
    Ok(ogdeveloper_core::schema_diff::prepare_schema_diff(options))
}

#[tauri::command]
#[allow(clippy::too_many_arguments)]
pub fn generate_schema_sync_sql(
    diffs: Vec<ogdeveloper_core::schema_diff::TableDiff>,
    function_diffs: Option<Vec<ogdeveloper_core::schema_diff::FunctionDiff>>,
    sequence_diffs: Option<Vec<ogdeveloper_core::schema_diff::SequenceDiff>>,
    rule_diffs: Option<Vec<ogdeveloper_core::schema_diff::RuleDiff>>,
    owner_diffs: Option<Vec<ogdeveloper_core::schema_diff::OwnerDiff>>,
    database_type: ogdeveloper_core::models::connection::DatabaseType,
    target_schema: Option<String>,
    target_sql_compatibility: Option<String>,
    cascade_delete: Option<bool>,
    source_dialect: Option<ogdeveloper_core::sql_dialect::descriptor::DialectKind>,
    field_mappings: Option<Vec<ogdeveloper_core::schema_diff::FieldMapping>>,
) -> Result<String, String> {
    Ok(ogdeveloper_core::schema_diff::generate_schema_sync_sql(
        &diffs,
        function_diffs.as_deref().unwrap_or_default(),
        sequence_diffs.as_deref().unwrap_or_default(),
        rule_diffs.as_deref().unwrap_or_default(),
        owner_diffs.as_deref().unwrap_or_default(),
        ogdeveloper_core::sql_dialect::ddl_profile::profile_for_connection(
            database_type,
            target_sql_compatibility.as_deref(),
        ),
        target_schema.as_deref(),
        cascade_delete.unwrap_or(false),
        source_dialect,
        &field_mappings.unwrap_or_default(),
    ))
}
