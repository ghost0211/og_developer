...
pub struct Connection<S, T> {
    ...
    state: State,
    /// ogdeveloper: server notice sink registered on the config.
    notice_sink: Option<std::sync::Arc<tokio::sync::broadcast::Sender<crate::NoticeMessage>>>,
    /// ogdeveloper: backend process id from BackendKeyData, tags notices.
    backend_pid: i32,
}
            ...
            let (mut messages, request_complete) = match message {
                BackendMessage::Async(Message::NoticeResponse(body)) => {
                    ...
                    if let Some(sink) = &self.notice_sink {
                        let _ = sink.send(crate::NoticeMessage {
