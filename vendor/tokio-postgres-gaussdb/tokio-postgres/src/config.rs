...
pub struct Config {
    ...
    /// ogdeveloper: server notices (RAISE NOTICE etc.) observed on every
    /// connection built from this config are forwarded here, tagged with the
    /// backend process id.
    pub(crate) notice_sink: Option<std::sync::Arc<tokio::sync::broadcast::Sender<crate::NoticeMessage>>>,
}
...
impl Config {
    ...
    /// ogdeveloper: registers a sink that receives every server notice
    /// (RAISE NOTICE/WARNING/INFO/…) observed on connections built from
    /// this config, tagged with the backend process id.
    pub fn notice_sink(&mut self, sink: std::sync::Arc<tokio::sync::broadcast::Sender<crate::NoticeMessage>>) -> &mut Config {
        self.notice_sink = Some(sink);
        self
    }
