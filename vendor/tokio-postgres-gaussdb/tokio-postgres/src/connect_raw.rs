    ...
    T: TlsConnect<S>,
{
    ...
    // ogdeveloper: forward server notices to the config-registered sink.
    let notice_sink = config.notice_sink.clone();
    let connection = Connection::new(stream.inner, stream.delayed, parameters, receiver, notice_sink, process_id);
