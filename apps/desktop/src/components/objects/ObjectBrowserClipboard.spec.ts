...
describe("ObjectBrowser table clipboard context menu", () => {
  ...
  it("normalizes copied and target schemas before paste validation", () => {
    ...
    expect(objectBrowserSource).toMatch(/function normalizeObjectBrowserTableClipboardSchema[\s\S]*?connectionStore\.getConfig\(connectionId\) \?\? props\.connection[\s\S]*?!isSchemaAware\(connection\.db_type\)[\s\S]*?return undefined/);
  });
