...
// Watch parameters to sync into editor
watch(
...
async function refreshParameters() {
  ...
  // Initialize editor with fallback/initial SQL immediately so the editor is never empty while loading
  await initEditor(generatedSql.value);
...
async function initEditor(text: string) {
  ...
  const dialect = createDbxCodeMirrorSqlDialect(langSql, "postgres", resolvedDatabaseType.value || "opengauss");
...
async function runTestExecution() {
  ...
  try {
    ...
    toast(t("contextMenu.executeSuccess", "执行完成"), 1500);
    ...
    serverLogs.value.push(`[ERROR] ${errorMsg}`);
    activeTab.value = "output";
    toast(errorMsg, 5000);
  } finally {
...
function openInSqlEditor() {
  ...
  if (!sql) return;
  const tabId = queryStore.createTab(props.connectionId, props.database, `Execute - ${props.routineName}`, "query", props.schema);
...
<template>
  <div class="flex h-full w-full flex-col min-h-0 bg-background text-foreground select-text">
    <!-- Main Action Toolbar -->
    <div class="flex items-center justify-between border-b bg-muted/40 px-3 py-1.5 text-xs shrink-0 select-none">
      <div class="flex items-center gap-1.5">
        ...
        <Button v-if="isOpenGaussRoutine" variant="outline" size="sm" class="h-7 gap-1.5 px-2.5" :disabled="executing || loading" @click="startDebugging">
        ...
        <Pane :size="100 - splitpanesSize" min-size="25" class="flex flex-col min-h-0 border-t bg-background">
          <Tabs v-model="activeTab" class="flex h-full flex-col min-h-0">
            ...
            <TabsContent value="variables" class="m-0 flex-1 min-h-0 overflow-y-auto p-0">
              <div v-if="loading" class="flex items-center justify-center p-8 text-sm text-muted-foreground gap-2 select-none">
              ...
              <div v-else-if="parameters.length" class="w-full">
                <table class="w-full text-left text-xs border-collapse font-sans">
                  <thead class="sticky top-0 bg-muted/90 backdrop-blur z-10 border-b text-muted-foreground font-medium select-none">
                    <tr>
                      <th class="py-2 px-3 w-[160px]">{{ t("contextMenu.parameterName") }}</th>
                      <th class="py-2 px-3 w-[130px]">{{ t("contextMenu.parameterType") }}</th>
                      <th class="py-2 px-3 w-[70px]">{{ t("contextMenu.parameterMode") }}</th>
                      <th class="py-2 px-3">{{ t("contextMenu.parameterValue") }}</th>
                      <th class="py-2 px-3 w-[180px]">输出回显 (Out Result)</th>
                  ...
                  <tbody class="divide-y font-mono">
                    <tr v-for="parameter in parameters" :key="`${parameter.ordinal}:${parameter.name}`" class="hover:bg-muted/40 transition-colors">
                      <td class="py-1.5 px-3 font-medium text-foreground truncate font-sans" :title="parameter.name">
                        {{ parameter.name }}
                      </td>
                      <td class="py-1.5 px-3 text-muted-foreground truncate" :title="parameter.dataType">
                        {{ parameter.dataType || "-" }}
                      </td>
                      ...
                      <td class="py-1.5 px-3">
                        <span v-if="outputResults[parameter.name] !== undefined" class="font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded text-xs inline-block max-w-[170px] truncate" :title="outputResults[parameter.name]">
