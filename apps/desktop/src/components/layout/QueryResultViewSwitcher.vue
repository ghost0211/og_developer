...
import { BarChart3, ListChecks, TerminalSquare } from "@lucide/vue";
import { useI18n } from "vue-i18n";
import { Button } from "@/components/ui/button";
import LightTooltip from "@/components/ui/LightTooltip.vue";

type OutputView = "result" | "output" | "summary" | "explain" | "chart";
...
<template>
  <div data-query-result-view-switcher class="flex shrink-0 items-center gap-1 px-1">
    ...
    <LightTooltip :text="t('tabs.output')" :disabled="!compact" side="bottom" :delay="0" :close-delay="0" nowrap>
      <Button
        size="sm"
        ...
        :title="t('tabs.output')"
        :aria-label="t('tabs.output')"
        :aria-pressed="activeView === 'output'"
        :disabled="!canShowOutput"
        @click="selectView('output')"
      >
        <TerminalSquare class="block h-3.5 w-3.5 self-center" />
        <span v-if="!compact" class="inline-flex h-4 items-center leading-none">{{ t("tabs.output") }}</span>
      </Button>
    </LightTooltip>
