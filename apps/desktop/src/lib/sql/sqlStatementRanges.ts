...
const ORACLE_LIKE_PL_SQL_DATABASES: ReadonlySet<DatabaseType> = new Set(["opengauss"]);
...
export function splitSqlStatementRanges(sql: string, databaseType?: DatabaseType, parameterOptions?: SqlParameterOptions): RawStatement[] {
  ...
  while (i < len) {
    ...
    if (ch === "$") {
      ...
      if (tagMatch) {
        markContent(i);
        if (statementStart !== -1 && startsWithPostgresDollarQuotedRoutinePrefix(sql.slice(statementStart, i))) {
    ...
    if (ch === ";") {
      ...
      if (typeof resolvedOraclePlSqlStatementEnd === "number") {
        markContent(i);
        if (i + 1 < resolvedOraclePlSqlStatementEnd) {
          i += 1;
          continue;
        }
        flush(i + 1);
...
function splitStatementRangeAtSoftStarts(sql: string, statement: RawStatement, databaseType?: DatabaseType, parameterOptions?: SqlParameterOptions): RawStatement[] {
  ...
  let currentExplainTargetKeyword = explainLikeTargetKeywordAt(sql, statement.from, parameterOptions);
  ...
  for (const lineStart of lineStarts) {
    ...
    currentExplainTargetKeyword = explainLikeTargetKeywordAt(sql, lineStart.from, parameterOptions);
...
function topLevelSoftStatementLineStarts(sql: string, statement: RawStatement, databaseType?: DatabaseType, parameterOptions?: SqlParameterOptions): Array<{ hitFrom: number; from: number; keyword: string }> {
  ...
  const unclosedExplainOptionsStart = explainOptionsStart !== null && skipBalancedParens(sql, explainOptionsStart, parameterOptions) === null ? explainOptionsStart : null;
...
function softStatementKeywordAt(sql: string, pos: number, databaseType?: DatabaseType, parameterOptions?: SqlParameterOptions): string | null {
  ...
  if (keyword === "COMMENT" && nextSqlWord(sql, pos + match[0].length, parameterOptions) !== "ON") return null;
...
function nextSqlWord(sql: string, pos: number, parameterOptions?: SqlParameterOptions): string | null {
  return nextSqlWordToken(sql, pos, parameterOptions)?.word ?? null;
}

function nextSqlWordToken(sql: string, pos: number, parameterOptions?: SqlParameterOptions): { word: string; end: number } | null {
  let i = pos;
...
function explainLikeTargetKeywordAt(sql: string, pos: number, parameterOptions?: SqlParameterOptions): string | null {
  const prefixMatch = /^[A-Za-z_][\w$]*/.exec(sql.slice(pos));
  ...
  if (prefix === "EXPLAIN" && sql[i] === "(") {
    const optionsEnd = skipBalancedParens(sql, i, parameterOptions);
...
function skipBalancedParens(sql: string, pos: number, parameterOptions?: SqlParameterOptions): number | null {
...
export function buildExecutionCandidates(sql: string, cursorPos: number, databaseType?: DatabaseType, parameterOptions?: SqlParameterOptions): SqlExecutionCandidate[] {
  ...
  if (!full) {
    return cursorStatement ? [candidateFromRange(cursorStatement, "cursor")] : [];
  }
  if (!cursorStatement) {
    return [candidateFromRange(full, "all")];
  }

  const sameContent = normalizeSql(cursorStatement.sql) === normalizeSql(full.sql);
  if (sameContent) {
    return [candidateFromRange(full, "all", ["cursor", "all"])];
  }

  return [candidateFromRange(cursorStatement, "cursor"), candidateFromRange(full, "all")];
}

function candidateFromRange(range: SqlTextRange, kind: SqlExecutionCandidate["kind"], supportedKinds: SqlExecutionCandidate["supportedKinds"] = [kind]): SqlExecutionCandidate {
  return {
    kind,
    supportedKinds,
