...
function buildVectorSystemPrompt(context: AiContext, mode: AiAssistantMode, custom?: CustomPromptContext): string {
  ...
  const referencedSqlFiles = formatReferencedSqlFiles(context);
  const lines: string[] = [
    isZh ? `你是 OG Developer 内置的向量数据库助手。当前连接的是 ${dbLabel(context.databaseType)} 数据库。用中文回复。` : `You are OG Developer's vector database assistant. Connected to ${dbLabel(context.databaseType)}. Reply in English.`,
