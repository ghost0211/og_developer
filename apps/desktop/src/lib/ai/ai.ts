...
function buildBasePromptLines(isZh: boolean): string[] {
  return [
    isZh ? "你是 ogdeveloper 内置的数据库助手。用中文回复。" : "You are ogdeveloper's built-in database assistant. Reply in English.",
...
function buildVectorSystemPrompt(context: AiContext, mode: AiAssistantMode, custom?: CustomPromptContext): string {
  ...
  const referencedSqlFiles = formatReferencedSqlFiles(context);
  const lines: string[] = [
    isZh ? `你是 ogdeveloper 内置的向量数据库助手。当前连接的是 ${dbLabel(context.databaseType)} 数据库。用中文回复。` : `You are ogdeveloper's vector database assistant. Connected to ${dbLabel(context.databaseType)}. Reply in English.`,
