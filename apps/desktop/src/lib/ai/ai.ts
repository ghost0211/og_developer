...
export function buildUserPrompt(action: AiAction, _context: AiContext, instruction: string, isZh: boolean): string {
...
export function buildSystemPrompt(action: AiAction, context: AiContext, mode: AiAssistantMode = "ask", custom?: CustomPromptContext): string {
  ...
  const isOpenGaussFamily = context.databaseType === "opengauss";
...
function buildBasePromptLines(isZh: boolean): string[] {
  return [
    isZh ? "你是 OG Developer 内置的数据库助手。用中文回复。" : "You are OG Developer's built-in database assistant. Reply in English.",
...
export function resolveAiNamespaceSelection(tab: QueryTab): AiNamespaceSelection {
...
export function resolveAiDatabaseTarget(tab: QueryTab, connection: ConnectionConfig): { database: string; schema?: string } {
  return { database: tab.database || connection.database || "main" };
}
