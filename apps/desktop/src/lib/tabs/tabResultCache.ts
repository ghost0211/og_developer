...
// Keep the physical legacy database until an atomic cross-database migration is validated.
...
function openCacheDb(): Promise<IDBDatabase | null> {
  ...
  dbPromise = new Promise((resolve) => {
    ...
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => {
      console.warn("[ogdeveloper][tab-result-cache:open:error]", request.error);
      resolve(null);
    };
...
async function writeRemoteRuntimeCache(key: string, bytes: Uint8Array, stats: { rowCount: number; columnCount: number }, ownerId?: string): Promise<boolean> {
  ...
  try {
    ...
    console.warn("[ogdeveloper][tab-result-cache:remote-write:error]", { key, error });
    return false;
  }
}
...
async function readRemoteRuntimeCache(key: string): Promise<Uint8Array | undefined> {
  ...
  try {
    ...
    console.warn("[ogdeveloper][tab-result-cache:remote-read:error]", { key, error });
    return undefined;
  }
}
...
async function deleteRemoteRuntimeCache(key: string): Promise<void> {
  ...
  try {
    ...
    console.warn("[ogdeveloper][tab-result-cache:remote-delete:error]", { key, error });
  }
}
...
export function resultCacheRuntimeConfig(tauriRuntime = isTauriRuntime(), environment = import.meta.env): ResultCacheRuntimeConfig {
  const configured = environment.VITE_OGDEVELOPER_RESULT_CACHE_BACKEND || environment.VITE_DBX_RESULT_CACHE_BACKEND;
  const primary = tauriRuntime || configured === "http" || configured === "runtime" ? "runtime" : "indexed-db";
  return { primary, fallbackEnabled: (environment.VITE_OGDEVELOPER_RESULT_CACHE_FALLBACK || environment.VITE_DBX_RESULT_CACHE_FALLBACK) !== "false" };
}
...
export async function writeResultCacheBackends(backends: ResultCacheBackend[], key: string, bytes: Uint8Array, stats: { rowCount: number; columnCount: number }, isCurrent: () => boolean = () => true, ownerId?: string): Promise<boolean> {
  for (const backend of backends) {
    ...
    try {
      ...
      console.warn("[ogdeveloper][tab-result-cache:write:error]", { key, backend: backend.name, error });
    }
  }
  return false;
}
...
export async function readResultCacheBackends(backends: ResultCacheBackend[], key: string): Promise<{ bytes: Uint8Array; backend: ResultCacheBackend } | undefined> {
  for (const backend of backends) {
    try {
      ...
      console.warn("[ogdeveloper][tab-result-cache:read:error]", { key, backend: backend.name, error });
    }
  }
  return undefined;
}
...
export async function deleteTabResultSnapshot(key: string): Promise<void> {
  ...
  try {
    ...
    console.warn("[ogdeveloper][tab-result-cache:delete:error]", { key, error });
    clearVersionLater();
  }
}
