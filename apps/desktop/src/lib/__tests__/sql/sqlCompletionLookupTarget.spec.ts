...
describe("sqlCompletionLookupTarget", () => {
  it("treats qualified table completion as a database lookup for database-qualified engines", () => {
  ...
  it("routes three-part table completion to the qualified database", () => {
    const target = resolveSqlCompletionTableLookupTarget({
      currentDatabase: "default_db",
      currentSchema: "public",
      supportsDatabaseQualifier: false,
  ...
  it("routes a database qualifier to schema completion", () => {
  ...
  it("does not mistake a current-database schema for a database", () => {
    const completionContext = getSqlCompletionContext("select * from public.", "select * from public.".length);
  ...
  it("prefers a current-database schema when a database has the same name", () => {
    ...
    expect(
      resolveSqlCompletionSchemaLookupDatabase({
        supportsDatabaseSchemaQualifier: true,
        knownDatabases: ["public", "Reporting"],
        knownSchemas: ["public", "sales"],
        completionContext,
      }),
    ).toBeUndefined();
  });
  ...
  it("keeps case-distinct database and schema completion names", () => {
    expect(mergeSqlCompletionQualifierNames(["Reporting", "public"], ["reporting", "public"])).toEqual(["Reporting", "public", "reporting"]);
  });
  ...
  it("resolves the completion scope to the current database and schema", () => {
    const sql = "SELECT * FROM T";
    const completionContext = getSqlCompletionContext(sql, sql.length, { databaseType: "postgres" });
    ...
    ).toEqual({ database: "app", schema: "public", completionContext });
  });

  it.each([
    ["SELECT public.fn_", "public", "fn_"],
    ["SELECT analytics.st_", "analytics", "st_"],
