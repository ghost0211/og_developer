...
export default withEnglishFallback({
  ...
  updates: {
    ...
    availableMessage: "ogdeveloper {latest} 已发布，当前版本为 {current}。",
    upToDate: "ogdeveloper 已是最新版本 ({version})。",
    failed: "检查更新失败：{error}",
    rateLimited: "GitHub 更新检查暂时触发频率限制。你仍然可以打开下载页手动查看。",
    openRelease: "打开下载页",
    downloadAndInstall: "下载并安装",
    activeTasksBlockUpdate: "有 {count} 个任务正在执行，请等待任务完成后再更新 ogdeveloper。",
    portableAutomaticUpdate: "ogdeveloper 将下载已签名的便携版 ZIP，退出后仅替换 ogdeveloper.exe 并自动重启。portable.ogdeveloper 和 data 会保留。",
  ...
  connection: {
    ...
    oracleTnsPathHint: "该路径由 ogdeveloper 后端/Oracle Agent 读取。桌面端填写本机路径，Web 和 Docker 部署填写服务器路径。",
    ...
    kafkaKerberosKeytabPlaceholder: "ogdeveloper Agent 所在机器路径，例如 /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "可选，ogdeveloper Agent 所在机器路径，例如 /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab 和 krb5.conf 路径由 ogdeveloper Agent 读取，必须存在于运行 ogdeveloper Agent 的机器上；不会从当前浏览器上传文件。",
    ...
    gaussdbConnectionModeNativeHint: "使用 ogdeveloper 内置的 PostgreSQL 兼容协议，适用于 GaussDB 的 A/PG/Oracle 兼容模式。",
    ...
    agentJavaTooOld: "该驱动需要 Java 21。请在驱动管理器中使用 ogdeveloper 托管 JRE 21，或选择 Java 21 可执行文件。",
    ...
    jdbcDocs: "查看 JDBC 文档",
    jdbcPluginHint: "先安装 ogdeveloper JDBC 插件。有外部依赖的驱动请从驱动管理使用 Maven 安装，或同时导入驱动及全部依赖 JAR。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」有未保存的 SQL 更改，退出 ogdeveloper 后将丢失。是否保存后再退出？",
    unsavedChangesAppCloseMultipleMessage: "当前有 {count} 个 SQL 标签未保存，当前已定位到「{title}」。退出 ogdeveloper 后将丢失，是否保存后再退出？",
  ...
  welcome: {
    title: "数据库工作台",
    subtitle: "选择左侧连接开始浏览结构，或直接创建查询标签页。ogdeveloper 支持多种数据库、查询历史、数据表格编辑和 AI SQL 助手。",
    connections: "连接",
    connected: "已连接",
    databaseTypes: "数据库类型",
  ...
  configExport: {
    ...
    cryptoUnavailable: "当前浏览器环境不支持加密配置导入，请使用 HTTPS、localhost 或 ogdeveloper 桌面版。",
  ...
  ai: {
    ...
    codexCliEnvReservedName: "{name} 由 ogdeveloper 用于 MCP Server，不能在这里设置。",
    cliPath: "{provider} 路径",
    ...
    cliEnvReservedName: "{name} 由 ogdeveloper 用于 MCP Server，不能在这里设置。",
    codexMcpRequiredTitle: "需要 ogdeveloper MCP Server",
    codexMcpRequiredDescription: "Codex CLI 通过 ogdeveloper MCP Server 访问数据库结构和查询工具。请先安装后再在 AI 助手中使用 Codex。",
    reasoningLevel: "推理级别",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "配置覆盖了由 ogdeveloper 管理的 MCP 环境变量。请从供应商配置中移除 DBX_MCP_* 变量。",
      claudeCodeNotAuthenticated: "Claude Code CLI 尚未登录。请在终端执行 `claude auth login` 后重试。",
      claudeCodeMcpConfigInvalid: "Claude Code 无法读取或解析 ogdeveloper 生成的 MCP 配置。请检查 Claude Code CLI 版本和可执行文件路径。",
      dbxMcpMissing: "未找到兼容的 ogdeveloper MCP Server。请在 设置 > MCP 中检查并重新安装或更新。",
      claudeCodeMcpStartupFailed: "Claude Code 已加载 MCP 配置，但无法启动 ogdeveloper MCP Server。请检查 设置 > MCP 和下方诊断详情。",
      claudeCodeCommandLineTooLong: "Windows 拒绝了过长的 Claude Code 命令。请更新 ogdeveloper，并使用生成的 MCP 配置文件重试。",
      ...
      piAgentEnvReserved: "配置覆盖了由 ogdeveloper 管理的 Pi 或 MCP 环境变量。请移除 DBX_PI_* 和 DBX_MCP_* 变量。",
      piAgentNotAuthenticated: "Pi Coding Agent 没有可用的已认证模型供应商。请先在 Pi 中配置或登录供应商。",
      piAgentMcpStartupFailed: "Pi 无法启动 ogdeveloper 的作用域 MCP 桥接。请检查 设置 > MCP 和下方诊断详情。",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "集群状态仍可正常查看。ogdeveloper 会直接访问当前 etcd endpoint 拼接后的 /metrics，请确认该地址可由 ogdeveloper 应用访问。",
  ...
  sqlParameters: {
    title: "SQL 参数",
    description: "为 SQL 模板占位符填写参数值，ogdeveloper 会在执行前替换为 SQL 字面量。",
    name: "参数名",
    syntax: "Syntax",
    type: "类型",
    value: "参数值",
  ...
  settings: {
    ...
    securityTab: "安全",
    aboutTab: "关于我们",
    aboutDescription: "openGauss 专用数据库开发工具。基于 ogdeveloper 构建，内置官方 JDBC 驱动，提供 PL/SQL 开发、调试与对象管理一体化体验。",
    fontFamily: "字体",
    dataGridFontFamily: "查询结果网格字体",
    dataGridFontFamilyDescription: "用于查询结果、表数据、列标题和单元格内编辑器。",
    uiFontFamily: "界面字体",
    uiFontAppDefault: "ogdeveloper 默认",
    ...
    iconThemeDefault: "默认",
    iconThemeDefaultDescription: "使用彩色 ogdeveloper 应用图标。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "应用内使用黑色 ogdeveloper 标识。",
    ...
    quitOnCloseDescription: "开启后，点击窗口关闭按钮将彻底退出 ogdeveloper，而不是隐藏到系统托盘。",
    ...
    updateNotificationsEnabledDescription: "关闭后，ogdeveloper 不会自动检查应用和驱动更新，也不会显示更新红点；仍可手动检查更新。",
    ...
    duckDbWorkerProcessIsolationDescription: "将 DuckDB 查询放到独立 worker 进程中运行。停止长查询时，如果 DuckDB 未及时响应取消请求，ogdeveloper 会结束 worker 进程以释放数据库文件锁。",
    duckDbWorkerMaxProcesses: "DuckDB 最大并发 worker 数",
    duckDbWorkerMaxProcessesDescription: "限制 ogdeveloper 同时运行的 DuckDB worker 进程数量。范围：1-16，默认：4。",
    duckDbWorkerProcessIsolationRestartRequired: "重启 ogdeveloper 后生效。",
    restartDbx: "重启 ogdeveloper",
    restartDbxFailed: "重启 ogdeveloper 失败：{error}",
    debugLogsCopy: "复制日志",
    debugLogsCopied: "已复制",
    ...
    openTabsRestoreModeDescription: "选择 ogdeveloper 启动时如何恢复上次打开的标签页。",
    ...
    jdbcPlugin: "ogdeveloper JDBC 插件",
    ...
    supportInfoAppVersion: "ogdeveloper 版本",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows: "无法删除旧的 JRE 目录：{path}\n可能的原因：\n  - 仍有 ogdeveloper Agent / java 进程占用该目录\n  - 防病毒软件正在扫描\n请关闭可能持有该目录的进程，或重启 ogdeveloper 后重试。\n（原始错误：{error}）",
    ...
    javaRuntimeManaged: "ogdeveloper 托管 JRE",
  ...
  databaseBackup: {
    title: "数据库备份",
    schedules: "备份计划",
    runtimeRequirement: "ogdeveloper 处于打开状态时会执行定时备份，包括最小化到托盘时。",
    addSchedule: "新建计划",
    editSchedule: "编辑计划",
