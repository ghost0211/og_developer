...
export default withEnglishFallback({
  ...
  ai: {
    ...
    cliMcpRequiredTitle: "需要 ogdeveloper MCP Server",
    cliMcpRequiredDescription: "{provider} 会通过 ogdeveloper MCP Server 访问数据库结构和查询工具。请先安装后再在 AI 助手中使用该提供商。",
    cliPath: "{provider} 路径",
  ...
  settings: {
    ...
    syncWebDavDescription: "将包含 SQL 库的 ogdeveloper 快照上传到兼容 WebDAV 的存储服务，或从远端恢复。",
    ...
    syncRemotePathDescription: "上传时 ogdeveloper 会自动创建缺失的父目录。",
    syncSecretNotice: "默认只同步连接信息和设置，不会同步数据库密码、SSH 密码、代理密码、HTTP 隧道 Token、连接串或 AI API Key。",
    syncSecrets: "同步加密后的敏感信息",
    syncSecretsDescription: "开启后，ogdeveloper 会先加密数据库密码、SSH 密码、代理密码、HTTP 隧道 Token 和 AI API Key，再上传到远端存储。",
    syncSecretsSharedDescription: "WebDAV 使用此密码加密包含的敏感信息。GitHub Gist 和 Gitee 代码片段使用独立的加密和敏感信息选项。",
    syncSecretsPassphrase: "同步密码",
    syncSecretsPassphraseDescription: "这个密码只用于加密和恢复敏感信息，ogdeveloper 不会保存；换设备恢复时需要再次输入。",
    syncTest: "测试",
    syncUpload: "上传",
    syncDownload: "下载",
    ...
    syncAutoUploadDescription: "仅在 ogdeveloper 打开时运行。下载仍需手动确认，避免意外覆盖本地保存的 SQL。",
    ...
    syncDownloadConfirm: "要下载并应用远端 ogdeveloper 快照吗？本地元信息和保存的 SQL 会被替换。",
    syncSnippetTitle: "GitHub / Gitee 代码片段同步",
    syncSnippetDescription: "将同一份 ogdeveloper 快照保存到私有代码片段。首次上传会自动创建代码片段并保存其 ID。",
    ...
    syncSnippetMigrateLegacyConfirm: "ogdeveloper 会先创建新的加密代码片段，再永久删除旧明文代码片段 {id}。此操作无法撤销，是否继续？",
    ...
    mcpTitle: "MCP Server",
    mcpDescription: "检查 Claude Code、Cursor 等编程助手使用的 ogdeveloper MCP Server 安装与版本状态。",
    mcpChecking: "检查中",
    ...
    mcpConfig: "MCP 配置",
    mcpConfigOptionsHint: "连接范围和执行权限均由 ogdeveloper 统一控制并对所有 MCP 客户端立即生效；生成配置只包含启动 MCP Server 和连接当前 ogdeveloper 所需的运行参数，不包含权限或连接范围变量。",
    mcpScopeConnection: "允许 MCP 访问的连接",
    mcpScopeConnectionDescription: "ogdeveloper 只向 MCP 暴露选中的连接，修改后对所有 MCP 客户端生效。",
    mcpScopeMode: "连接范围模式",
    ...
    mcpExecutionModeDescription: "选择 ogdeveloper 对所有 MCP 客户端强制执行的权限级别。客户端的读写权限配置无法更改此级别。",
    ...
    mcpExecutionModeReadOnlyDescription: "仅允许 ogdeveloper 判定为读取的请求；禁止已识别的数据库写入和连接管理变更。数据库账号权限仍是最终硬边界。",
    ...
    mcpNpmBoundary: "ogdeveloper 只检测和提示 MCP 状态；安装与升级仍由 npm 完成。",
    mcpRefresh: "重新检查",
    mcpGuide: "MCP 指南",
