...
export default withEnglishFallback({
  ...
  updates: {
    ...
    availableMessage: "ogdeveloper {latest} が利用可能です。現在のバージョンは {current} です。",
    upToDate: "ogdeveloper は最新です ({version})。",
    ...
    activeTasksBlockUpdate: "{count} 件のタスクが実行中です。完了してから ogdeveloper を更新してください。",
    portableAutomaticUpdate: "ogdeveloper は署名済みのポータブル ZIP をダウンロードし、終了後に ogdeveloper.exe のみを置き換えて自動的に再起動します。portable.ogdeveloper とデータは保持されます。",
  ...
  connection: {
    ...
    oracleTnsPathHint: "このパスは ogdeveloper バックエンド/Oracle Agent が読み取ります。デスクトップではローカルパス、Web/Docker ではサーバーパスを指定してください。",
    ...
    gaussdbConnectionModeNativeHint: "GaussDB の A/PG/Oracle 互換モードでは、ogdeveloper 内蔵の PostgreSQL 互換プロトコルを使用します。",
    ...
    advancedTab: "詳細",
    recognizeTab: "自動認識",
    recognizeHint: "既存の接続設定テキスト（JDBC URL、key=value 形式、任意の説明）を下に貼り付けると、デフォルトの大規模言語モデルが解析して「接続情報」フォームに自動入力します。",
    recognizePlaceholder: "例：jdbc:opengauss://192.168.1.10:5432/postgres?user=gaussdb&password=xxx、または host=... port=... user=... password=... database=...",
    recognizeButton: "認識して入力",
    recognizeNoConfig: "大規模言語モデルが設定されていません。先に「設定 → AI」で設定してください",
    recognizeEmptyInput: "認識する設定テキストを先に貼り付けてください",
    recognizeSuccess: "認識が完了し、接続情報に入力しました。確認して接続テストを行ってください",
    recognizeSuccessHint: "認識済み ✓ 確認して接続してください",
    recognizeInvalidOutput: "モデル出力に有効な JSON オブジェクトが含まれていません",
    ...
    kafkaKerberosKeytabPlaceholder: "ogdeveloper Agent が動作するマシンのパス。例: /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "オプション。ogdeveloper Agent が動作するマシンのパス。例: /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab と krb5.conf のパスは ogdeveloper Agent が読み取るため、ogdeveloper Agent が動作するマシン上に存在する必要があります。ブラウザからファイルをアップロードすることはありません。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」に未保存の SQL 変更があり、ogdeveloper の終了後に失われます。保存してから終了しますか？",
    unsavedChangesAppCloseMultipleMessage: "{count} 個の SQL タブが未保存です。現在「{title}」を表示中です。ogdeveloper の終了後に失われます。保存してから終了しますか？",
  ...
  grid: {
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "ogdeveloper はこの結果に対して安全に書き戻せる対象テーブルを 1 つに特定できませんでした。",
  ...
  ai: {
    ...
    codexMcpRequiredTitle: "ogdeveloper MCP Serverが必要です",
    ...
    knowledgeBaseEnabled: "openGauss ドキュメント知識ベース",
    knowledgeBaseEnabledDescription: "AI リクエストの前に組み込みの openGauss 公式ドキュメントを検索し、関連スニペットをプロンプトに注入して、回答が実際の openGauss 構文と動作に沿うようにします。",
    ...
    cliErrors: {
      ...
      claudeCodeMcpStartupFailed: "Claude CodeはMCP設定を読み込みましたが、ogdeveloper MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
    ...
    actions: {
      ...
      generatePlsql: "PL/SQL 生成",
      fixPlsqlError: "PL/SQL エラー修正",
  ...
  contextMenu: {
    ...
    createView: "ビューを作成",
    createJob: "ジョブを作成",
    createScheduler: "スケジューラを作成",
    editJob: "ジョブを編集",
    editScheduler: "スケジューラを編集",
    runJob: "今すぐ実行",
    enableJob: "有効化",
    disableJob: "無効化",
    runJobSuccess: "ジョブ「{name}」を実行しました",
    runJobFailed: "ジョブの実行に失敗しました：{message}",
    runJobTriggered: "ジョブ「{name}」をトリガーしました。スケジューラがまもなく実行します",
    runJobDisabled: "ジョブ「{name}」は無効です。先に有効化してください",
    runJobFailedGeneric: "実行に失敗しました。ジョブの失敗メッセージを確認してください",
    enableJobSuccess: "ジョブ「{name}」を有効化しました",
    disableJobSuccess: "ジョブ「{name}」を無効化しました",
    dropTable: "テーブルを削除",
    ...
    parameterDefaultHint: "DEFAULTがチェックされている場合、ogdeveloperはその引数を省略し、データベースがプロシージャのデフォルト値を使用します。",
    ...
    dropFunction: "関数を削除",
    dropSequence: "シーケンスを削除",
    dropSynonym: "シノニムを削除",
    dropPackage: "パッケージを削除",
    dropPackageBody: "パッケージ本体を削除",
    dropType: "型を削除",
    dropTypeBody: "型本体を削除",
    dropJob: "ジョブを削除",
    dropScheduler: "スケジューラを削除",
    ...
    confirmDropSequenceMessage: "シーケンス「{name}」を削除しますか？",
    confirmDropSynonymTitle: "シノニムを削除",
    confirmDropSynonymMessage: "シノニム「{name}」を削除しますか？",
    confirmDropPackageTitle: "パッケージを削除",
    confirmDropPackageMessage: "パッケージ「{name}」を削除しますか？",
    confirmDropTypeTitle: "型を削除",
    confirmDropTypeMessage: "型「{name}」を削除しますか？",
    confirmDropJobTitle: "ジョブを削除",
    confirmDropJobMessage: "ジョブ「{name}」を削除しますか？",
    confirmDropSchedulerTitle: "スケジューラを削除",
    confirmDropSchedulerMessage: "スケジューラ「{name}」を削除しますか？",
    dropViewSuccess: "ビュー「{name}」を削除しました",
    dropProcedureSuccess: "プロシージャ「{name}」を削除しました",
    dropFunctionSuccess: "関数「{name}」を削除しました",
    dropSequenceSuccess: "シーケンス「{name}」を削除しました",
    dropSynonymSuccess: "シノニム「{name}」を削除しました",
    dropPackageSuccess: "パッケージ「{name}」を削除しました",
    dropPackageBodySuccess: "パッケージ本体「{name}」を削除しました",
    dropTypeSuccess: "型「{name}」を削除しました",
    dropTypeBodySuccess: "型本体「{name}」を削除しました",
    dropJobSuccess: "ジョブ「{name}」を削除しました",
    dropSchedulerSuccess: "スケジューラ「{name}」を削除しました",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "クラスタ状態は引き続き利用できます。ogdeveloper は現在の etcd エンドポイントへ直接 /metrics を要求するため、生成された URL に ogdeveloper アプリケーションから到達できることを確認してください。",
  ...
  plDebug: {
    ...
    refresh: "更新",
    stop: "デバッグ停止",
    currentLine: "現在行 {line}",
    locals: "変数",
    noLocals: "変数なし",
    backtrace: "コールスタック",
    breakpoints: "ブレークポイント",
    noBreakpoints: "行番号の左側をクリックしてブレークポイントを設定",
    constVar: "定数は変更できません",
    editVar: "クリックして値を編集",
    setVarFailed: "変数 {name} の設定に失敗しました",
  },
  about: {
  ...
  sqlParameters: {
    title: "SQLパラメーター",
    description: "SQLテンプレートのプレースホルダーに値を入力します。ogdeveloperは実行前にSQLリテラルへ置換します。",
    name: "パラメーター",
    syntax: "Syntax",
    type: "型",
    value: "値",
  ...
  settings: {
    ...
    uiFontAppDefault: "ogdeveloper デフォルト",
    ...
    showTrayIconDescription: "無効時はアイコンが表示されませんが、ウィンドウを閉じると従来通りogdeveloperはバックグラウンドに隠れます。",
    ...
    updateNotificationsEnabledDescription: "無効時、ogdeveloperはアプリやドライバーのアップデートを自動確認せず、アップデートバッジも表示しません。手動確認は引き続き利用可能です。",
    ...
    duckDbWorkerProcessIsolationDescription: "DuckDB クエリを独立した worker プロセスで実行します。長時間クエリがキャンセルに応答しない場合、ogdeveloper は worker プロセスを停止してデータベースファイルのロックを解放できます。",
    duckDbWorkerMaxProcesses: "DuckDB の最大同時 worker 数",
    duckDbWorkerMaxProcessesDescription: "ogdeveloper が同時に実行できる DuckDB worker プロセス数を制限します。範囲: 1-16。既定値: 4。",
    duckDbWorkerProcessIsolationRestartRequired: "この変更を有効にするには ogdeveloper を再起動してください。",
    restartDbx: "ogdeveloper を再起動",
    restartDbxFailed: "ogdeveloper の再起動に失敗しました: {error}",
    ...
    openTabsRestoreModeDescription: "ogdeveloper 起動時に前回開いていたタブをどのように復元するかを選択します。",
    ...
    jdbcPlugin: "ogdeveloper JDBCプラグイン",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows:
      "古い JRE ディレクトリを削除できませんでした: {path}\n考えられる原因:\n  - ogdeveloper Agent / java プロセスがディレクトリを使用中です\n  - ウイルス対策ソフトがスキャン中です\nディレクトリを使用している可能性のあるプロセスを終了するか、ogdeveloper を再起動して再試行してください。\n（元のエラー: {error}）",
  ...
  mqExchanges: {
    ...
    virtualHost: "仮想ホスト",
    namePlaceholder: "例: my-events",
    cancel: "キャンセル",
    create: "作成",
    bind: "バインド",
    bindDialogTitle: "{name} にバインド",
    queueNamePlaceholder: "例: my-queue",
    exchangeNamePlaceholder: "例: my-events",
