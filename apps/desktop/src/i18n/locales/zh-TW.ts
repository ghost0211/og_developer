...
export default withEnglishFallback({
  ...
  settings: {
    ...
    syncWebDavDescription: "上傳包含 SQL 庫的 ogdeveloper 快照到相容 WebDAV 的儲存服務，或從該服務還原快照。",
    ...
    syncRemotePathDescription: "上傳時 ogdeveloper 會自動建立缺失的父目錄。",
    syncSecretNotice: "預設只同步連線詳細資料和設定。資料庫密碼、SSH 密碼、代理伺服器密碼、連線字串和 AI API Key 會保留在本機。",
    syncSecrets: "同步加密後的敏感資訊",
    syncSecretsDescription: "開啟後，ogdeveloper 會先加密資料庫密碼、SSH 密碼和 AI API Key，再上傳到 WebDAV。",
    syncSecretsPassphrase: "同步密碼",
    syncSecretsPassphraseDescription: "此密碼只用於加密和還原敏感資料。ogdeveloper 不會儲存；每台裝置都需要再次輸入。",
    syncTest: "測試",
    syncUpload: "上傳",
    syncDownload: "下載",
    ...
    syncAutoUploadDescription: "僅在 ogdeveloper 開啟時執行。下載仍需手動確認，避免意外覆蓋本機已儲存的 SQL。",
    ...
    syncDownloadConfirm: "要下載並套用遠端 ogdeveloper 快照嗎？本機 metadata 和已儲存的 SQL 會被取代。",
    apply: "套用",
    applyAndClose: "套用並關閉",
    reset: "重設",
    resetDefaults: "還原預設",
    ...
    mcpTab: "MCP",
    mcpTitle: "MCP Server",
    mcpDescription: "檢查 Claude Code、Cursor 等程式助理使用的 ogdeveloper MCP Server 安裝與版本狀態。",
    mcpChecking: "檢查中",
    ...
    mcpConfig: "MCP 設定",
    mcpConfigOptionsHint: "連線範圍與執行權限均由 ogdeveloper 統一控制，並對所有 MCP 用戶端立即生效；產生的設定只包含啟動 MCP Server 與連線目前 ogdeveloper 所需的執行參數，不包含權限或連線範圍變數。",
    mcpScopeConnection: "允許 MCP 存取的連線",
    mcpScopeConnectionDescription: "ogdeveloper 只向 MCP 公開選取的連線，變更後對所有 MCP 用戶端生效。",
    mcpScopeMode: "連線範圍模式",
    ...
    mcpExecutionModeDescription: "選擇 ogdeveloper 對所有 MCP 用戶端強制執行的權限等級。用戶端的讀寫權限設定無法變更此等級。",
    ...
    mcpExecutionModeReadOnlyDescription: "僅允許 ogdeveloper 判定為讀取的請求；禁止已識別的資料庫寫入與連線管理變更。資料庫帳號權限仍是最終硬邊界。",
    ...
    mcpNpmBoundary: "ogdeveloper 只檢測和提示 MCP 狀態；安裝與升級仍由 npm 完成。",
    mcpRefresh: "重新檢查",
    mcpGuide: "MCP 指南",
    ...
    syncSnippetMigrateLegacyConfirm: "ogdeveloper 會先建立新的加密程式碼片段，再永久刪除舊明文程式碼片段 {id}。此操作無法復原，是否繼續？",
