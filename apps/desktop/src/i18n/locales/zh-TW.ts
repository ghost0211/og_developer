...
export default withEnglishFallback({
  ...
  updates: {
    ...
    availableMessage: "ogdeveloper {latest} 已發布，目前版本為 {current}。",
    upToDate: "ogdeveloper 已是最新版本 ({version})。",
    failed: "檢查更新失敗：{error}",
    rateLimited: "GitHub 更新檢查暫時觸發頻率限制。你仍然可以開啟下載頁手動檢視。",
    openRelease: "開啟下載頁",
    downloadAndInstall: "下載並安裝",
    activeTasksBlockUpdate: "有 {count} 個任務正在執行，請等待任務完成後再更新 ogdeveloper。",
    portableAutomaticUpdate: "ogdeveloper 將下載已簽署的可攜版 ZIP，結束後僅替換 ogdeveloper.exe 並自動重新啟動。portable.ogdeveloper 和 data 會保留。",
  ...
  connection: {
    ...
    oracleTnsPathHint: "此路徑由 ogdeveloper 後端/Oracle Agent 讀取。桌面版填寫本機路徑，Web 和 Docker 部署填寫伺服器路徑。",
    ...
    gaussdbConnectionModeNativeHint: "使用 ogdeveloper 內建的 PostgreSQL 相容協定，適用於 GaussDB 的 A/PG/Oracle 相容模式。",
    ...
    agentJavaTooOld: "此驅動程式需要 Java 21。請在驅動程式管理器中使用 ogdeveloper 託管 JRE 21，或選擇 Java 21 可執行檔。",
    ...
    advancedTab: "進階",
    recognizeTab: "自動辨識",
    recognizeHint: "將既有的連線設定文字（JDBC URL、鍵值對或任意描述）貼到下方，預設大模型會自動解析並填入「連線資訊」表單。",
    ...
    recognizeButton: "辨識並填入",
    recognizeNoConfig: "尚未設定大模型，請先在「設定 → AI」中設定",
    recognizeEmptyInput: "請先貼上需要辨識的設定文字",
    recognizeSuccess: "辨識成功，已自動填入連線資訊，請核對後測試連線",
    recognizeSuccessHint: "已辨識 ✓ 請核對後連線",
    recognizeInvalidOutput: "模型輸出中未包含有效的 JSON 物件",
    ...
    jdbcDocs: "檢視 JDBC 文件",
    jdbcPluginHint: "先安裝 ogdeveloper JDBC 外掛程式，再匯入資料庫廠商提供的 JDBC 驅動程式 JAR。",
    ...
    kafkaKerberosKeytabPlaceholder: "ogdeveloper Agent 所在機器路徑，例如 /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "可選，ogdeveloper Agent 所在機器路徑，例如 /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab 和 krb5.conf 路徑由 ogdeveloper Agent 讀取，必須存在於執行 ogdeveloper Agent 的機器上；不會從當前瀏覽器上傳檔案。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」有未儲存的 SQL 變更，結束 ogdeveloper 後將遺失。儲存後再結束嗎？",
    unsavedChangesAppCloseMultipleMessage: "目前有 {count} 個 SQL 分頁未儲存，目前已定位到「{title}」。結束 ogdeveloper 後將遺失，儲存後再結束嗎？",
  ...
  grid: {
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "ogdeveloper 無法為此結果唯一確定一個安全的回寫目標資料表。",
      ...
      "metadata-unavailable": "ogdeveloper 無法載入資料表 metadata，因此已停用結果編輯。",
    },
  ...
  configExport: {
    ...
    cryptoUnavailable: "目前瀏覽器環境不支援加密設定匯入，請使用 HTTPS、localhost 或 ogdeveloper 桌面版。",
  ...
  ai: {
    ...
    proxyUrl: "代理伺服器地址",
    permissionLevel: "Agent 資料庫權限",
    permissionLevelReadonly: "唯讀（預設）",
    permissionLevelData: "資料讀寫",
    permissionLevelFull: "完全（含 DDL）",
    permissionLevelDescription: "允許 AI Agent 在 Agent 模式下直接存取設定的資料庫。唯讀僅允許 SELECT/SHOW/EXPLAIN；資料讀寫增加 INSERT/UPDATE/DELETE；完全權限再允許 DDL（CREATE/ALTER/DROP/TRUNCATE）。生產資料庫無論此設定如何都強制唯讀。",
    ...
    knowledgeBaseEnabled: "openGauss 文件知識庫",
    knowledgeBaseEnabledDescription: "AI 請求前檢索內建的 openGauss 官方文件，將相關片段注入提示詞，讓回答符合 openGauss 真實的語法與行為。",
    ...
    codexMcpRequiredDescription: "Codex CLI 會透過 ogdeveloper MCP Server 存取資料庫結構與查詢工具。請先安裝後再於 AI 助理中使用 Codex。",
    ...
    codexCliEnvReservedName: "{name} 由 ogdeveloper 用於 MCP Server，不能在這裡設定。",
    cliPath: "{provider} 路径",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "設定覆寫了由 ogdeveloper 管理的 MCP 環境變數。請從供應商設定中移除 DBX_MCP_* 變數。",
      claudeCodeNotAuthenticated: "Claude Code CLI 尚未登入。請在終端機執行 `claude auth login` 後重試。",
      claudeCodeMcpConfigInvalid: "Claude Code 無法讀取或解析 ogdeveloper 產生的 MCP 設定。請檢查 Claude Code CLI 版本和可執行檔路徑。",
      dbxMcpMissing: "找不到相容的 ogdeveloper MCP Server。請在 設定 > MCP 中檢查並重新安裝或更新。",
      claudeCodeMcpStartupFailed: "Claude Code 已載入 MCP 設定，但無法啟動 ogdeveloper MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
      claudeCodeCommandLineTooLong: "Windows 拒絕了過長的 Claude Code 命令。請更新 ogdeveloper，並使用產生的 MCP 設定檔重試。",
      ...
      piAgentEnvReserved: "設定覆寫了由 ogdeveloper 管理的 Pi 或 MCP 環境變數。請移除 DBX_PI_* 與 DBX_MCP_* 變數。",
      piAgentNotAuthenticated: "Pi Coding Agent 沒有可用的已驗證模型供應商。請先在 Pi 中設定或登入供應商。",
      piAgentMcpStartupFailed: "Pi 無法啟動 ogdeveloper 的作用域 MCP 橋接。請檢查 設定 > MCP 和下方診斷詳情。",
    ...
    actions: {
      ...
      generatePlsql: "產生 PL/SQL",
      fixPlsqlError: "修復 PL/SQL 錯誤",
  ...
  contextMenu: {
    ...
    createView: "建立檢視",
    createJob: "建立作業",
    createScheduler: "建立排程",
    editJob: "編輯作業",
    editScheduler: "編輯排程",
    runJob: "立即執行",
    enableJob: "啟用",
    disableJob: "停用",
    runJobSuccess: "作業「{name}」已成功執行",
    runJobFailed: "作業執行失敗：{message}",
    runJobTriggered: "作業「{name}」已觸發，排程器將立即執行",
    runJobDisabled: "作業「{name}」已停用，請先啟用",
    runJobFailedGeneric: "執行失敗，請查看作業失敗訊息",
    enableJobSuccess: "作業「{name}」已啟用",
    disableJobSuccess: "作業「{name}」已停用",
    dropTable: "刪除資料表",
    emptyTable: "清空資料",
    truncateTable: "截斷資料表",
    ...
    confirmDropSequenceTitle: "刪除序列",
    confirmDropSequenceMessage: "確定要刪除序列「{name}」嗎？",
    confirmDropSynonymTitle: "刪除同義詞",
    confirmDropSynonymMessage: "確定要刪除同義詞「{name}」嗎？",
    confirmDropPackageTitle: "刪除套件",
    confirmDropPackageMessage: "確定要刪除套件「{name}」嗎？",
    confirmDropTypeTitle: "刪除型別",
    confirmDropTypeMessage: "確定要刪除型別「{name}」嗎？",
    confirmDropJobTitle: "刪除作業",
    confirmDropJobMessage: "確定要刪除作業「{name}」嗎？",
    confirmDropSchedulerTitle: "刪除排程",
    confirmDropSchedulerMessage: "確定要刪除排程「{name}」嗎？",
    dropViewSuccess: "檢視「{name}」已刪除",
    dropProcedureSuccess: "預存程序「{name}」已刪除",
    dropFunctionSuccess: "函式「{name}」已刪除",
    dropSequenceSuccess: "序列「{name}」已刪除",
    dropSynonymSuccess: "同義詞「{name}」已刪除",
    dropPackageSuccess: "套件「{name}」已刪除",
    dropPackageBodySuccess: "套件本體「{name}」已刪除",
    dropTypeSuccess: "型別「{name}」已刪除",
    dropTypeBodySuccess: "型別本體「{name}」已刪除",
    dropJobSuccess: "作業「{name}」已刪除",
    dropSchedulerSuccess: "排程「{name}」已刪除",
  ...
  plDebug: {
    ...
    refresh: "重新整理",
    stop: "停止偵錯",
    currentLine: "目前行 {line}",
    locals: "變數",
    noLocals: "無變數",
    backtrace: "呼叫堆疊",
    breakpoints: "中斷點",
    noBreakpoints: "點擊程式碼行號左側可設中斷點",
    constVar: "常數不可修改",
    editVar: "點擊編輯變數值",
    setVarFailed: "設定變數 {name} 失敗",
  },
  about: {
    repository: "GitHub 存放庫",
  },
  dangerDialog: {
    title: "危險操作",
  ...
  sqlParameters: {
    title: "SQL 參數",
    description: "為 SQL 模板佔位符填寫參數值，ogdeveloper 會在執行前替換為 SQL 字面量。",
    name: "參數名",
    syntax: "Syntax",
    type: "類型",
    value: "參數值",
  ...
  settings: {
    ...
    uiFontFamily: "介面字型",
    uiFontAppDefault: "ogdeveloper 預設",
    ...
    iconThemeDefault: "預設",
    iconThemeDefaultDescription: "使用彩色 ogdeveloper 應用程式圖示。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "應用程式內使用黑色 ogdeveloper 標誌。",
    ...
    quitOnCloseDescription: "開啟後，點擊視窗關閉按鈕將徹底退出 ogdeveloper，而不是隱藏到系統匣。",
    ...
    updateNotificationsEnabledDescription: "關閉後，ogdeveloper 不會自動檢查應用程式和驅動程式更新，也不會顯示更新紅點；仍可手動檢查更新。",
    ...
    duckDbWorkerProcessIsolationDescription: "將 DuckDB 查詢放到獨立 worker 進程中執行。停止長查詢時，如果 DuckDB 未及時回應取消請求，ogdeveloper 會結束 worker 進程以釋放資料庫檔案鎖。",
    duckDbWorkerMaxProcesses: "DuckDB 最大並行 worker 數",
    duckDbWorkerMaxProcessesDescription: "限制 ogdeveloper 同時執行的 DuckDB worker 進程數量。範圍：1-16，預設：4。",
    duckDbWorkerProcessIsolationRestartRequired: "重啟 ogdeveloper 後生效。",
    restartDbx: "重啟 ogdeveloper",
    restartDbxFailed: "重啟 ogdeveloper 失敗：{error}",
    debugLogsCopy: "複製日誌",
    debugLogsCopied: "已複製",
    ...
    openTabsRestoreModeDescription: "選擇 ogdeveloper 啟動時如何還原上次開啟的分頁。",
    ...
    jdbcPlugin: "ogdeveloper JDBC 外掛程式",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows: "無法刪除舊的 JRE 目錄：{path}\n可能的原因：\n  - 仍有 ogdeveloper Agent / java 程序占用該目錄\n  - 防毒軟體正在掃描\n請關閉可能持有該目錄的程序，或重新啟動 ogdeveloper 後重試。\n（原始錯誤：{error}）",
    ...
    javaRuntimeManaged: "ogdeveloper 託管 JRE",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "叢集狀態仍可正常檢視。ogdeveloper 會直接存取目前 etcd endpoint 拼接後的 /metrics，請確認 ogdeveloper 應用程式可以存取該位址。",
  ...
  databaseBackup: {
    title: "資料庫備份",
    schedules: "備份計畫",
    runtimeRequirement: "ogdeveloper 處於開啟狀態時會執行定時備份，包含最小化到托盤時。",
    addSchedule: "新建計畫",
    editSchedule: "編輯計畫",
